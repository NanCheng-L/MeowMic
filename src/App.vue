...
const { startDenoising, stopDenoising, updateConfig, listInputDevices, listOutputDevices } = useAudioEngine()
...
const handleStart = async () => {
  ...
  if (!selectedInput.value || !selectedOutput.value) return
  isLoading.value = true
  try {
    await startDenoising(selectedInput.value, selectedOutput.value)
...
watch([selectedInput, selectedOutput], async () => {
  saveConfig()
  if (isRunning.value) {
    await handleStop()
    await handleStart()
  ...
  <div class="app">
    ...
    <main class="main">
      ...
      <section class="section visualization">
        <div class="meters">
          <AudioMeter label="输入" :level="stats.input_level" />
          <AudioMeter label="输出" :level="stats.output_level" />
        </div>
      ...
      <section class="section stats-bar">
        <div class="stat">
          <span class="stat-label">降噪量</span>
